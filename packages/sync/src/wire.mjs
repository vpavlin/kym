// The Delivery wire envelope (docs/data-model.md §8). One event per EVENT
// message; the bytes here are what gets sealed (encrypted) and handed to the
// transport. Large bulk backfill uses the CHUNK envelope (not needed for
// per-event live sync — events are tiny, well under Waku's 150 KB cap).
