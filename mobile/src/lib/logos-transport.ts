...
// NOTE: only per-app divergence from qaku's copy — KYM has @noble/hashes 2.2.0 (sha256
// in sha2.js); qaku has 1.8.0 (/sha256). Unify when this becomes a real shared package.
...
// FLEET — logos.test (cluster 2). Moved off logos.dev, which migrated to CLUSTER 3
// while liblogosdelivery still maps logos.dev→cluster 2, so a fresh node dials the
// now-cluster-3 boxes with cluster-2 config and never meshes ("existing connections
// persist, new ones fail"). logos.test stays cluster 2, keeping KYM's shard (7) valid.
// Keep preset + entryNodes in lockstep with kym_core + the hub (KYM_DELIVERY_CFG).
