...
- **`mobile/`** — the **companion app** (React Native / Expo, Android-first). Its one job: **capture an expense in under 10 seconds** — amount-first quick-add plus **on-device receipt OCR** (snap → prefilled amount, merchant, date). Also a full offline budget view, transaction edit/delete, multiple budgets, and share/join. Syncs when it can.
