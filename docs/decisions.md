...
Intent: the desktop module is the full budgeting surface; the phone is fast capture. **Reality today: the module is a read-only viewer; the CLI is the full surface and mobile is the richer editor.** Closing this (a desktop editor UI + missing SLOTs) is the top parity gap. **Status: open.** See `parity.md` (#TBD).
